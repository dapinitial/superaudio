...
## 2026-07-09 — M12 AP2 control plane verified on a real Apple TV; modern tvOS requires PTP (not NTP)

With an Apple TV 4K on the LAN (2026-07-05), the whole AirPlay 2 control plane was built fresh-Swift and verified against real hardware (AppleTV11,1, AirTunes/950.7.1) over 2026-07-08→09: SRP pair-setup **M1–M6** (long-term Ed25519 keys exchanged + persisted), **pair-verify** (X25519 — PIN-free after the one-time pairing), a **ChaCha20-Poly1305 encrypted control channel**, and **two-phase RTSP SETUP** (receiver returns the RTP `dataPort`). Commits d6f1c15 → 8d0834b.

**The decision/finding that matters:** modern tvOS **rejects the legacy NTP timing path** AirPlay 1 uses — a SETUP declaring `timingProtocol: "NTP"` returns 400 *after full processing* (proven by the `X-Apple-ProcessingTime` header), while `timingProtocol: "PTP"` + `timingPeerInfo` returns 200. So the AP2 audio path **must implement a real PTP (IEEE-1588) timing peer** — it cannot reuse the AP1 NTP-channel code. This is now the M12 critical path (est. ~1,200–1,500 lines: UDP PTP peer + clock discipline + RTP audio to the dataPort). Sync-model note: sender-side broadcaster delay (gotcha #16) still works, but PTP time is the receiver's clock — we follow it, we don't shift it. New gotchas #29–#32 capture the SETUP-verb, IPv6-URI-400, X-Apple-HKP-470, and real-sender-header traps hit along the way. Transient pairing was retested after the SRP fix and still auth-fails on this Apple TV → **PIN (HomeKit) pairing is the verified path**; supersedes the "transient is the streaming path" assumption in the 2026-06-25 (M12 sprint) entry.

Also this session — **pricing/GTM decision:** kill the $5-per-protocol addon IAP model (keep the `LicenseManager` architecture as future optionality only); ship a **single price** — early-bird $12 → list $19.99 → ~$14.99 via a coupon/credit incentive. Rationale: the niche is volume-constrained (awareness + free AirConnect substitute) not price-constrained, and support cost per sale is fixed + high, so a permanent $9.99 floor is a trap; a low *intro* price gets the low-regret psychology without capping the ceiling. Monetization is realistically lifestyle/side-income (~$10k–$40k/yr, decaying) — build it because it's wanted and the AP2 IP is valuable, treat revenue as gravy. (A planned "wrap AirConnect for effortless onboarding" idea was rejected: AirConnect is GPL/libraop-lineage and would poison the proprietary binary — the exact thing our license hygiene forbids — and iOS can't capture system audio anyway; the license-clean version is an iOS *remote/setup companion* for the Mac app.)

...
- [ ] **Pick a customer-facing product name off "SuperAudio" (flagged risky 2026-07-09) — MARKETING name only, NOT a codebase rename.** The internal SPM modules / bundle ID / file headers can stay `SuperAudio*` forever (customers never see them); changing them is cosmetic churn with real risk and no benefit. Only the *retail* name matters here: the menu-bar/`CFBundleName` display string + the marketing site + the domain. "SuperAudio" is risky as a retail name — too generic to protect and crowded: a **same-name Mac App Store app** (id1156321898), the audio studio superaudio.studio, the "Super Audio Cart" VST, and Sony's "Super Audio CD"/SACD adjacency; **superaudio.com** (held since 1995) and **superaudio.app** (registered Jan 2026, live) are both taken. This is **deferrable** — it blocks marketing spend, not the listen test / Dev Program / code. When chosen (David's call): grab the exact-match `.com` and run a USPTO Class 9/42 knockout search before any print/marketing spend. (Note: "Spacelab" is just the A5/A7 AirPlay device names, not a decided brand — do not assume it.)

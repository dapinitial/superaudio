...
Currently working on (2026-06-29): **M6.5 Claude Skill alpha** — the `/onboard-audio-device` skill is BUILT and pipeline-verified end-to-end on the local LAN (probe → identify → draft → validate → install-to-overlay; PR step gated off until the profiles repo splits public). Scope is AP1 sink-role (the only path SuperAudio consumes today). Lives in `skills/onboard-audio-device/` (SKILL.md + `probe-raop.py` / `validate-profile.py` / `install-profile.sh` / `open-pr.sh` + AP1 template). Shipped alongside it: **M5.5 matcher fix** — `ProfileStore` now folds model-bearing TXT keys (`am`, `model`) into the text `match.modelHints` resolves against, so community profiles can key on the *model* (`am=A5`) instead of the owner's pet name. The live probe confirmed the A5 advertises `am=A5` and that pre-fix matching only worked via the personal "Spacelab" hint. **Next: the on-hardware <90s demo** (force-unmatch the A5 → skill re-onboards → log flips to "matched" → plays tuned). **M12 AirPlay 2 sender** remains built + offline-verified but **blocked on a HomeKit AP2 device** (no Apple TV/HomePod on the LAN; the Sonos One SL needs the MFi path). Other unblocked threads: M5.5 remaining slices (AP1 codec/latency, Sonos volume — slice 1 shipped), passive-sync / GCC-PHAT, M6 polish, M7 launch prep. Recent (2026-06-25→30): M6.6 Sonos grouping (read+write), zombie-silence auto-recovery, diagnostics-panel upgrade, M5.5 slice 1, et=1 verified-broken, **profiles-repo split public** (`dapinitial/superaudio-device-profiles`, consumed as a pinned submodule; drift-guarded), and **ProfileStore live reload** (2026-06-30 — FSEvents overlay watch; a dropped-in profile matches with no app restart, closing the Skill-install relaunch seam).
...
- **M12 AirPlay 2 sender** ← V1 priority. **~90% built (2026-07-09), all fresh Swift/CryptoKit — NO vendored `pair_ap`, no `swift-opus` (realtime uses raw PCM).** Done + hardware-verified vs a real Apple TV: pairing (M1–M6) + pair-verify + encrypted channel + two-phase RTSP SETUP + PTP grandmaster + RTP audio streaming (SETPEERS/SETRATEANCHORTIME accepted). **Remaining: confirm audible sound (listen test + pcap) — the last tuning is PTP master/slave + ChaCha nonce padding (gotchas #33–#35), then wire AP2 in as a 4th `AudioSink`.** Unlocks HomePods + modern Sonos + smart TVs + modern AVRs.
...
- [x] **Profile loader** (`DeviceProfileLoader.swift`) — loads built-in profiles from app bundle; overlays any from `~/Library/Application Support/SuperAudio/Profiles/`; per-file errors logged, never crashes. Plus `ProfileStore` (2026-06-26) — app-wide singleton that loads once and resolves a discovered `SinkDescriptor` → `DeviceProfile` (memoized), the seam protocol modules consume.
  - **✅ Matcher fix SHIPPED (2026-06-29) — match on the model, not the pet name.** `ProfileStore.modelHintText(for:)` now feeds `displayName` **plus** the model-bearing TXT keys (`am`, `model`) into `matchProfile`, where before it passed only `displayName` (for RAOP, the owner-given name after the `@` in `{MAC}@{Name}`). The schema always documented `modelHints` as matching "the device's mDNS TXT record" but the consumer ignored it — so a community "B&W A5" profile keyed on the model would not resolve on a downstream user who named their speaker something else. Live probe confirmed the A5 advertises `am=A5`; the seed had only matched via the personal "Spacelab" hint. Allowlisted keys only (flag fields like `et`/`cn`/`sf` excluded); cross-protocol bleed stays gated by `bonjourServiceType`. **This is the prerequisite that makes the M6.5 Skill's drafted profiles portable.** Verified on hardware 2026-06-29: with all personal name-hints purged from the A5/A7 seeds, the A5 (user-named "Spacelab Audio") matches `bowers-wilkins-a5` purely via `am=A5`.
  - **✅ Live reload SHIPPED (2026-06-30) — no restart to pick up a new profile.** `ProfileStore` watches the user-overlay directory (`~/Library/Application Support/SuperAudio/Profiles/`) with FSEvents (`kFSEventStreamCreateFlagFileEvents`, covering both a fresh file copied in and an in-place edit); on any change it reloads bundle+overlay and drops its per-`SinkID` match cache, so the next lookup sees the new profile. `profiles`/`loadErrors` became lock-guarded snapshots (were immutable `let`); file I/O runs outside the lock, only the swap + cache-clear is guarded. **This closes the relaunch seam the Skill's install step used to require** — drop a profile in and it matches on the next session start, live. Verified on hardware 2026-06-30: A5 streaming, a probe profile copied into the overlay triggered a 4→5 reload + cache clear with audio never dropping; removal reloaded back to 4.
  - [ ] **Follow-up — match firmware fields, not the user-name (structured matching).** A speaker's Bonjour name is 100% user-chosen and brand-agnostic ("Spacelab Audio", "Kitchen"), so a name-derived hint is not just useless but *dangerous*: it can **false-match** a different device the owner happened to name similarly (e.g. a non-B&W `_raop._tcp` speaker named "…A5…" inheriting the A5's quirks). Step 0's current fold (displayName + `am` into one substring blob) *enables* `am` matching but inherits that risk, because a model-code hint like "A5" can still hit the name half. The robust version matches hints against the **`am`/firmware fields structurally** (and MAC OUI), with the user-name as a weak last-resort only. Codify the rule in the Skill's draft guidance + schema notes: hints must be manufacturer-set model identifiers, never user-chosen names. Purged the seed name-hints already (A5 ✅; A7 set to `am`-inferred "A7", pending hardware confirmation that the A7 advertises `am=A7`).
...
- [x] **Public MIT-licensed GitHub repo** (LIVE 2026-06-29): [`github.com/dapinitial/superaudio-device-profiles`](https://github.com/dapinitial/superaudio-device-profiles) (Homebrew-formulae model — pure data, no DB). Seeded with the four built-ins, JSON schema, `scripts/validate.py` + a `Validate profiles` GitHub Action that validates submitted PRs, and CODEOWNERS. The monorepo consumes it as a **pinned git submodule** at `superaudio-device-profiles/`; `sync_device_profiles.sh` mirrors it into the bundled SPM resource, and `Scripts/check_profile_drift.sh` (wired into `build_app.sh`) fails the build on drift (kills gotcha #28). The Skill's `open-pr.sh` PR step is now ungated (opt-in via `SUPERAUDIO_ENABLE_PR=1`).
...
**Gate:** the four currently-hard-coded protocol quirks (verbatim-ALAC rejection, empty-metadata-required, et=0-default, source-port-matching) all live in JSON profiles loaded at runtime. A user can drop a hand-authored profile into `~/Library/Application Support/SuperAudio/Profiles/` and have the app pick it up live (FSEvents-watched; no restart) on the next match. The `superaudio-device-profiles` repo is public on GitHub with the four built-ins and a working CI.
...
> **✅ ALPHA BUILT (2026-06-29) — AP1 sink-role, local-overlay loop, pipeline-verified on the LAN.** Lives in `skills/onboard-audio-device/`: `SKILL.md` (phase script + privacy disclosure rules), `scripts/probe-raop.py` (passive `dns-sd` browse/resolve → JSON with an `am`/`pk`-based `likelyKind` heuristic that steers away from Macs and AP2 devices on `_raop._tcp`), `scripts/validate-profile.py` (zero-dependency schema validation), `scripts/install-profile.sh` (writes to the overlay dir `ProfileStore` reads), `scripts/open-pr.sh` (gated off until the repo splits public), and `templates/airplay1-receiver.json`. **Scope deliberately narrowed for the alpha to AP1 sink-role** — the only path SuperAudio consumes today (M5.5 slice 1). The capture/subscribe/soundbar-control phases below are deferred (no `tcpdump`; AP1 wire format is known, so match hints + volume scale + `et` are derivable from discovery alone). Drafts key on the `am` model identifier (portable) thanks to the M5.5 matcher fix. **Verified end-to-end**: live probe correctly classified the A5 (`airplay1-receiver`), the Sonos One (`airplay2-likely`), and an iMac (`computer`); draft → validate PASS → install-to-overlay → undo all work; PR gate correctly refuses. Remaining: the on-hardware match-flip demo, then broaden scope (capture/control-role) post-alpha.

- [ ] Bundled as a local skill in the app's resources (works without internet); also available as a standalone skill installable into the user's Claude Code if they have it. *(Alpha lives in-repo at `skills/onboard-audio-device/`; app-resource bundling is M7/M8 packaging.)*
...
- [x] **Public repo go-live** (2026-06-29) — [`github.com/dapinitial/superaudio-device-profiles`](https://github.com/dapinitial/superaudio-device-profiles) with MIT license, JSON schema, four seed profiles, `Validate profiles` CI for PR validation, CODEOWNERS. Consumed by the monorepo as a pinned submodule; drift-guarded via `check_profile_drift.sh`.
...
*Last updated: 2026-06-30 — ProfileStore LIVE RELOAD shipped (`ProfileStore` FSEvents-watches the user-overlay dir; on any profile add/edit/remove it reloads bundle+overlay and drops the per-`SinkID` match cache, so a dropped-in profile matches with no app restart — closes the Skill-install relaunch seam; verified on hardware during an A5 soak, 4→5→4 reloads with audio never dropping). Earlier 2026-06-29 — M6.5 Claude Skill alpha BUILT (`/onboard-audio-device` in `skills/onboard-audio-device/`: probe/identify/draft/validate/install-to-overlay; AP1 sink-role; PR step gated until repo split; pipeline-verified on the LAN); M5.5 matcher fix (`ProfileStore` folds TXT `am`/`model` into modelHints matching so community profiles key on the model, not the owner's pet name — live-confirmed the A5 advertises `am=A5` and had only matched via the personal "Spacelab" hint). Earlier 2026-06-27 — M6.6 Sonos group awareness read+write (feed coordinator only; one-tap group/ungroup, local SOAP — gotcha #24); zombie-silence auto-recovery (re-attach detached process tap — gotcha #26); diagnostics-panel upgrade (silence/gap counts, live Sonos topology, encryption mode); M5.5 device profiles now consumed (ProfileStore + AP1 volume scale from JSON — slice 1); et=1 verified BROKEN (silent on A5/A7, et=0 is the path — gotcha #27); M12 AP2 foundation built + offline-verified (module, `_airplay._tcp` discovery, TLV8, SRP-6a/CryptoKit) but BLOCKED on a HomeKit AP2 device (no Apple TV/HomePod on the LAN; the Sonos One SL needs the MFi path). GOTCHAS now 27 entries. Earlier 2026-06-03 — cross-protocol sync first reached on real hardware (B&W A5 + Sonos Playbar + Sonos One SL) via Mic Calibrate, later found stable only as a static by-ear offset with correctors off; new M6.8 `PassiveSyncMonitor` milestone (passive content-based continuous sync; GCC-PHAT = #1 next lever, chirp re-anchor interim, ultrasonic pilot endgame); rotary knob UI (#100) marked REMOVED/cancelled (AirPlay delay slider now 25 ms steps); current frontier = M12 AP2 sender (committed V1 hero, hardware on the LAN) + the passive-sync/GCC-PHAT thread. Earlier 2026-05-17 (Sunday) — M6 sync arc DONE (sender-side broadcaster delay, handshake barrier, Sonos start-align defer, supervisor retry cap, session-race nonce guard, volume debounce, TEARDOWN timeout, Mac-mute fix, audio-gap telemetry, Sonos Δ slider rip, idempotent Play All, Restart All); M3 signal-handler item marked done; new M6.3 Sync UX milestone added with 5 tasks (#98–#102) including Mac-mic auto-calibration as M11 Path A precursor; M11 reframed to acknowledge Path A ships in M6.3. Earlier 2026-05-16 (night, same-night expansion) — M5.5 schema gains dual roles (sink + control); M6.5 Claude Skill scope expanded to onboard soundbars + control-layer devices alongside speakers; M15 Optical Hub gains IR remote-learning hardware ($0.50 BOM addition); 5-SKU lineup; M14 ↔ M15 reorder under consideration. Earlier 2026-05-16 update: M13/M14 hardware sizing + LATAM mfg strategy. Earlier 2026-05-15 (night) — M5 done + M6 partial.*

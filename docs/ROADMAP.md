...
Currently working on (2026-06-27): **M12 AirPlay 2 sender** foundation is built + offline-verified (module, `_airplay._tcp` discovery, TLV8, SRP-6a/CryptoKit; `/info` reaches a real device) but is **blocked on a HomeKit AP2 device** — there is **NO Apple TV 4K or HomePod on the LAN**; the only AP2 device is a Sonos One SL, which needs the MFi/FairPlay path, not the HomeKit `pair-setup` we built. Resume when an Apple TV/HomePod is available. Meanwhile, plenty is unblocked: **M5.5** remaining slices (codec params, Sonos volume — slice 1 shipped), the **passive-sync / GCC-PHAT** thread, M6 polish, M6.5 Claude Skill, M7 launch prep. Recent (2026-06-25→27): M6.6 Sonos grouping (read+write), zombie-silence auto-recovery, diagnostics-panel upgrade, M5.5 slice 1, et=1 verified-broken.

...
- 🟡 **M12 — AirPlay 2 sender (committed V1 hero).** AP2 is in V1 scope, not a non-goal. **Control plane DONE + hardware-verified against a real Apple TV 4K (AppleTV11,1, AirTunes/950.7.1, 2026-07-08/09):** full SRP pair-setup **M1–M6** (CryptoKit + BigInt), **pair-verify** (X25519, PIN-free after a one-time pairing), a **ChaCha20-Poly1305 encrypted control channel**, and **two-phase RTSP SETUP** — both phases 200 OK, the receiver hands back the `dataPort` we push RTP to. The `SuperAudioAirPlay2` module, `_airplay._tcp` discovery, TLV8 codec, and fresh-Swift crypto (no vendored `pair_ap`; see DECISIONS.md 2026-06-25) all verified live. **Audio pipeline BUILT + STREAMING (2026-07-09):** a fresh-Swift PTP (IEEE-1588) grandmaster, type-96 realtime RTP sender (ChaCha20-Poly1305), and the full orchestration (`AP2AudioSession`) now run end-to-end — system audio is captured, converted 48k f32 → 44.1k s16, packetized, and streamed to the receiver's `dataPort` at the correct realtime rate, with SETPEERS and SETRATEANCHORTIME both accepted (`200`). Dev loop: `--ap2-pair` / `--ap2-verify` / `--ap2-setup` / `--ap2-play` (see [docs/AP2-PAIRING-RUNBOOK.md](docs/AP2-PAIRING-RUNBOOK.md)). **Key finding: modern tvOS requires PTP, not the legacy NTP path** (SETUP 400s on NTP, 200s on PTP — gotcha #29). **NOT YET CONFIRMED: audible sound out of the speaker** — packets transmit correctly, but a listen test + likely a packet-capture pass remains to resolve the last tuning (PTP master/slave — the Apple TV also emits Sync; ChaCha nonce padding; RECORD times out). See gotchas #33–#35.
